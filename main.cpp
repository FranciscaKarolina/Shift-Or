...
int shift_or(char *text, int textlen, char *pattern, int patlen) {//recebe 4 parâmetros: o texto a ser pesquisado (text),
    //o comprimento do texto (textlen), o padrão a ser encontrado (pattern) e o comprimento do padrão (patlen)
    ...
    //se um caractere do texto corresponder ao caractere correspondente no padrão,
    //o bit correspondente na máscara de bits será definido como 1.
    for (int i = 0; i < m; ++i)
...
int main() { //lê a sequência de DNA e padrão de pesquisa da entrada do usuário e
    //chama a função shift_or para buscar o padrão na sequência.
